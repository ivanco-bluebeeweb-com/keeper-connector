"""Connection lifecycle for Keeper Connector."""
...
@chat.function(
    "connect_keeper",
    "Connect Keeper account via credentials.",
    action_type="write",
    chain_callable=True,
    event="keeper-connector.connect_keeper",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_keeper(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Keeper Connector."""
    client = KeeperClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Keeper Account",
...
@chat.function(
    "list_connections",
    "List connected Keeper accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_keeper",
    "Disconnect Keeper account.",
    action_type="write",
    chain_callable=True,
    event="keeper-connector.disconnect_keeper",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_keeper(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
